...
def get_project_root():
    ...
    project_markers = ['README.md', 'package.json', 'pyproject.toml', 'requirements.txt', '.git']

    # If current directory has project markers, use it
    if any(marker.exists() for marker in [base / m for m in project_markers]):
        return base

    # Try going up one level (useful if script is in a subdirectory)
    parent = base.parent
    if any(marker.exists() for marker in [parent / m for m in project_markers]):
        return parent

    # Try going up two levels
    grandparent = parent.parent
    if any(marker.exists() for marker in [grandparent / m for m in project_markers]):
        return grandparent
...
def main():
    ...
    base = get_project_root()
    print(f"Working with project root: {base}")
    os.chdir(base)
